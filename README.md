...
- ~~**Conformance suite**~~ — **delivered**, [`conformance/`](conformance) and [SPEC.md](SPEC.md) §25.3. 33 cases covering proofs, v2 reports and manifests, leaf-v2 profiles, group memberships, coverage pairings, anchor chains and evidence packs, each with an expected outcome and a declared feature set. All three implementations run it, so it pins the *decisions* the format requires rather than only the bytes it produces.
