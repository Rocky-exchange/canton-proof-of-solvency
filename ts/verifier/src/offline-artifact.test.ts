...
import { renderOfflineVerifier } from "../scripts/build-offline.mjs";

/**
 * The offline page is checked in so a user can download one file and run it
 * with their connection off. These tests are what keep that promise true: it
 * must stay in step with the source, and it must not reach the network.
 */
const checkedIn = readFileSync(
  fileURLToPath(new URL("../../../offline/verifier.html", import.meta.url)),
  "utf8"
);

describe("offline verifier artifact", () => {
  it("is in step with the source", async () => {
    expect(
      checkedIn,
      "offline/verifier.html is stale — rebuild with `npm run build:offline`"
    ).toBe(await renderOfflineVerifier());
  });
  ...
  it("uses no network API", () => {
    for (const api of ["fetch(", "XMLHttpRequest", "WebSocket", "importScripts", "navigator.send"]) {
      expect(checkedIn, `found ${api}`).not.toContain(api);
    }
  });

  it("embeds the verification logic rather than reimplementing it", () => {
    // Domain strings are the load-bearing constants; if they are absent the
    // page is not running the real verifier.
  ...
  it("keeps the provenance vocabulary the page is built around", () => {
    expect(checkedIn).toContain("recomputed here");
    expect(checkedIn).toContain("publisher says");
  });

  /**
   * The one failure the node tests cannot see: the script asks for an element
   * the markup does not define, and the page dies on load.
   */
  it("only addresses element ids that the markup defines", () => {
    const entry = readFileSync(
      fileURLToPath(new URL("./offline-entry.ts", import.meta.url)),
      "utf8"
    );
